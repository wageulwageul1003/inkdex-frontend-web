...
import { usePostFindPasswordEmailCertification } from '@/hooks/auth/find/password/usePostFindPasswordEmailCertification';
import { usePutFindPasswordEmailCertificationCheck } from '@/hooks/auth/find/password/usePutFindPasswordEmailCertificationCheck';
...
const FindPasswordEmail = () => {
  ...
  const { mutateAsync: postFindPasswordEmailCertification } =
    usePostFindPasswordEmailCertification(); // 인증번호 요청
  const { mutateAsync: putFindPasswordEmailCertificationCheck } =
    usePutFindPasswordEmailCertificationCheck(); // 인증번호 확인
  ...
  const startTimer = async () => {
    ...
    try {
      const result = await postFindPasswordEmailCertification({
          ...
          description: (
            <div className="flex w-full flex-col">
              <span className="font-body1 text-center text-gray-500">
                입력하신 아이디와 이메일이 일치하지 않습니다. 가입 시 입력한
  ...
  const handleVerifyCertNum = async () => {
    ...
    try {
      // 인증번호 확인 요청
      const result = await putFindPasswordEmailCertificationCheck({
        account: form.getValues('account'),
        email: form.getValues('email'),
