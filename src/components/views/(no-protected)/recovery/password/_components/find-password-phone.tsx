...
import { usePostFindPasswordPhoneCertification } from '@/hooks/auth/find/password/usePostFindPasswordPhoneCertification';
import { usePutFindPasswordPhoneCertificationCheck } from '@/hooks/auth/find/password/usePutFindPasswordPhoneCertificationCheck';
import { useAlert } from '@/hooks/useAlert';
import { getThirtyMinutesFromNow } from '@/lib/utils';
...
const FindPasswordPhone = () => {
  ...
  const { mutateAsync: postFindPasswordPhoneCertification } =
    usePostFindPasswordPhoneCertification(); // 인증번호 요청
  const { mutateAsync: putFindPasswordPhoneCertificationCheck } =
    usePutFindPasswordPhoneCertificationCheck(); // 인증번호 확인
  ...
  const onSubmit = () => {
    router.push(`/recovery/password/find`);
  };
  ...
  const startTimer = async () => {
    ...
    try {
      const result = await postFindPasswordPhoneCertification({
          ...
          description: (
            <div className="flex w-full flex-col">
              <span className="font-body1 text-center text-gray-500">
                입력하신 아이디와 휴대번호가 일치하지 않습니다. 가입 시 입력한
  ...
  const handleVerifyCertNum = async () => {
    ...
    try {
      // 인증번호 확인 요청
      const result = await putFindPasswordPhoneCertificationCheck({
        account: form.getValues('account'),
        phone: form.getValues('phone'),
      ...
      if (result) {
        ...
        clearErrors('certificationNumber');
        setButtonText('인증 완료');
        Cookies.set(CERTIFICATION_TOKEN, result.data.passwordToken, {
